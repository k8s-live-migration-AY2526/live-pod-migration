...
kubectl wait --for=condition=ready pod -l app=csi-nfs-node -n kube-system
kubectl wait --for=condition=ready pod -l app=csi-nfs-controller -n kube-system
...
kubectl wait --for=jsonpath='{.status.phase}'=Bound pvc/checkpoint-repo -n live-pod-migration-controller-system
